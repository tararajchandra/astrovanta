        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="p-4 text-center text-gray-500">Loading clients...</td>
                </tr>
              ) : error ? (
                <tr>
                  <td colSpan={5} className="p-4 text-center text-red-500">Error loading clients.</td>
                </tr>
              ) : customers?.length === 0 ? (
                <tr>
                ...
                customers?.map((customer) => (
                  <tr key={customer.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer">
                    ...
                    <td className="p-4">
                      <Link to={`/clients/${customer.id}`} className="text-indigo-600 hover:underline text-sm font-medium">
                        View Details
                      </Link>
                    </td>
                  </tr>
