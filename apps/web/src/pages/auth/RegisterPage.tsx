...
export function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    if (error) {
      setError(error.message || 'রেজিস্ট্রেশন ব্যর্থ হয়েছে।');
    } else {
      navigate('/book');
    }
    setLoading(false);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-10" style={{ background: 'linear-gradient(135deg, #0f0c29 0%, #302b63 50%, #24243e 100%)' }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-white/50">বিনামূল্যে রেজিস্ট্রেশন করুন</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8">
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            <div>
              <label className="block text-white/70 text-sm font-medium mb-2">পুরো নাম</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="block text-white/70 text-sm font-medium mb-2">ফোন নম্বর</label>
              <input
                type="tel"
                value={form.phone}
          ...
          <div className="mt-6 text-center">
            <span className="text-white/40 text-sm">অ্যাকাউন্ট আছে? </span>
            <Link to="/login" className="text-yellow-400 font-medium text-sm hover:text-yellow-300">
              লগইন করুন
            </Link>
          </div>
        </div>
